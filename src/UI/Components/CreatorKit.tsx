...
export const CreatorKit = () => {
  const { products, ownProducts } = useComponentStore();
  const { envAssets, modifyEnvAsset, activeAssetId, setActiveAssetId } =
    useEnvAssetStore();
  ...
  const [envProduct, setEnvProduct] = useState<EnvProduct | undefined>(
    undefined
  );
  const [envAsset, setEnvAsset] = useState<EnvAsset | undefined>(undefined);
  const [envOwnProduct, setEnvOwnProduct] = useState<EnvProduct | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    if (entityType === "PRODUCT" && activeProductId !== null) {
      ...
      setEnvOwnProduct(undefined);
    } else if (entityType === "ASSET" && activeAssetId !== null) {
      setEnvAsset(envAssets[activeAssetId]);
      setEnvProduct(undefined);
      setEnvOwnProduct(undefined);
    } else if (entityType === "OWNASSET" && activeOwnProductId !== null) {
      setEnvOwnProduct(ownEnvProducts[activeOwnProductId]);
      setEnvAsset(undefined);
      setEnvProduct(undefined);
    }
  }, [
      ...
      >
        <Button
          sx={{
            ...
            backgroundColor:
              entityType === "PRODUCT" ? "rgb(77, 177, 255)" : "transparent",
            "&:hover": {
              backgroundColor:
                entityType === "PRODUCT"
                  ? "rgb(77, 177, 255)"
          ...
          onClick={() => {
            ...
            if (activeOwnProductId !== null) setOwnActiveProductId(null);
        ...
        <Button
          sx={{
            ...
            backgroundColor:
              entityType === "ASSET" ? "rgb(77, 177, 255)" : "transparent",
            "&:hover": {
              backgroundColor:
                entityType === "ASSET"
                  ? "rgb(77, 177, 255)"
        ...
        <Button
          sx={{
            ...
            fontSize: "16px",
            textTransform: "none",
            color: entityType === "OWNASSET" ? "white" : "rgb(77, 177, 255)",
            backgroundColor:
              entityType === "OWNASSET" ? "rgb(77, 177, 255)" : "transparent",
            "&:hover": {
              backgroundColor:
                entityType === "OWNASSET"
                  ? "rgb(77, 177, 255)"
          ...
          onClick={() => {
            if (activeProductId !== null) setActiveProductId(null);
            if (activeAssetId !== null) setActiveAssetId(null);
            if (entityType !== "OWNASSET") setEntityType("OWNASSET");
          }}
          className="OwnStoreAssetButton"
        >
          Own Asset
        </Button>
      </Box>
  ...
  const handleCheckboxChange = (
    ...
    } else if (entityType === "OWNASSET") {
      const ownProduct: Product | undefined = ownProducts.find(
        (product) => product.id === parameters.productId
      );
      if (!ownProduct) return;
      const isProductFirstTime =
        ownEnvProducts[ownProduct.id]?.imageIndex === undefined &&
        ownEnvProducts[ownProduct.id]?.modelIndex === undefined;
      if (event.target.checked && isProductFirstTime) {
        setToolType("MEDIA");
        setMediaType("PHOTO");
        setOwnActiveProductId(ownProduct.id);
        // Preload its models
        ownProduct.models.forEach((model) => {
          useGLTF.preload(model.sources?.[0].url || "");
        });
      } else {
        setOwnActiveProductId(null);
        setToolType(null);
      }
      const ownEnvProduct = Object.values(ownEnvProducts).find(
        (envProduct) => envProduct.id === ownProduct.id
      );

      const newOwnEnvProduct: EnvProduct = {
        id: ownProduct.id,
        isEnvironmentProduct: event.target.checked,
        imageIndex: isProductFirstTime ? 0 : ownEnvProduct?.imageIndex,
      };
      modifyOwnEnvProduct(ownProduct.id, newOwnEnvProduct);
      ...
      >
        <Button
          sx={{
            ...
            backgroundColor:
              paramsType === "CUSTOM" ? "rgb(77, 177, 255)" : "transparent",
            "&:hover": {
              backgroundColor:
                paramsType === "CUSTOM"
                  ? "rgb(77, 177, 255)"
            ...
            if (entityType === "PRODUCT" && envProduct) {
              ...
              if (envProduct.placeHolderId !== undefined && placeHolderData) {
                const placeHolder = placeHolderData.find(
                  (placeHolder) => placeHolder.id === envProduct.placeHolderId
                );
  ...
  const ThreeParamsEditor = () => {
      ...
      useEffect(() => {
        const handleMouseMove = (event: MouseEvent) => {
          ...
          if (threeParamsSlider.current) {
            ...
            if (
              type === sliderValues[0] &&
              (type === "SCALE" || axis === sliderValues[1])
            ) {
              const SENSITIVITY = 3;
        ...
        const handleMouseDown = (event: MouseEvent) => {
          ...
          threeParamsSlider.current =
            type + (type !== "SCALE" ? " " + axis : "");
            ...
            placeHolderData.map((placeHolderItem) => {
              const placeHolderEntity = Object.values(envProducts).find(
                (envProduct: EnvProduct) =>
                  envProduct.placeHolderId === placeHolderItem.id
              );
              ...
              return (
                <span
                  ...
                  ref={(el) =>
                    (placeHolderItemRefs.current[placeHolderItem.id] = el)
                  }
                  style={{
                    width: "100%",
                  }}
    ...
    return (
      ...
      >
        {paramsType === "CUSTOM" && <CustomParamterEntries />}
  ...
  // OwnProduct List
  const ownProductListRef = useRef<HTMLDivElement>(null);
  const ownProductItemRefs = useRef<{ [id: number]: HTMLSpanElement | null }>(
    {}
  );
              ...
              >
                <Box
                  sx={{
                    ...
                    justifyContent: "start",
                    alignItems: "center",
                    backgroundColor: envProducts[product.id]
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        opacity: envProducts[product.id]?.isEnvironmentProduct
                          ? 1
                          : 0.2,
                        cursor: envProducts[product.id]?.isEnvironmentProduct
                          ? "pointer"
                          : "arrow",
                      },
                    }}
    ...
    return (
      !activeOwnProductId && (
        <Box
          ...
          ref={ownProductListRef}
          className="OwnProductList"
        >
          {ownProducts.map((product) => {
            return (
              <span
                key={product.id}
                ref={(el) => (ownProductItemRefs.current[product.id] = el)}
                style={{
                  width: "100%",
                }}
              >
                <Box
                  sx={{
                    ...
                    justifyContent: "start",
                    alignItems: "center",
                    backgroundColor: ownEnvProducts[product.id]
                      ?.isEnvironmentProduct
                      ? "rgb(10, 10, 10)"
                      : "rgb(5, 5, 5)",
                    padding: "0 15px 0 15px",
                ...
                >
                  <Checkbox
                    ...
                    checked={
                      ownEnvProducts[product.id]?.isEnvironmentProduct || false
                    }
                    onChange={(event) => {
                      handleCheckboxChange(event, { productId: product.id });
                    }}
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      backgroundColor: "rgb(255, 255, 255)",
                      objectFit: "contain",
                      opacity: ownEnvProducts[product.id]?.isEnvironmentProduct
                        ? 1
                        : 0.5,
                    }}
                  />
                  <Typography
                    sx={{
                      ...
                      fontFamily: "'Poppins', sans-serif",
                      fontWeight: "normal",
                      color: ownEnvProducts[product.id]?.isEnvironmentProduct
                        ? "rgba(255, 255, 255, 0.83)"
                        : "rgba(255, 255, 255, 0.25)",
                      textAlign: "left",
                      textOverflow: "ellipsis",
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      height: "20px",
                      opacity:
                        product.id === activeOwnProductId &&
                        toolType === "MEDIA"
                          ? 1
                          : ownEnvProducts[product.id]?.isEnvironmentProduct
                          ? 0.5
                          : 0.2,
                      "&:hover": {
                        opacity: ownEnvProducts[product.id]
                          ?.isEnvironmentProduct
                          ? 1
                          : 0.2,
                        cursor: ownEnvProducts[product.id]?.isEnvironmentProduct
                          ? "pointer"
                          : "arrow",
                      },
                    }}
                    onClick={() => {
                      if (ownEnvProducts[product.id]?.isEnvironmentProduct) {
                        setMediaType(
                          ownEnvProducts[product.id]?.type || "PHOTO"
                        );
                        if (product.id === activeOwnProductId) {
                          if (toolType !== "MEDIA") {
                            setToolType("MEDIA");
                          } else {
                            setOwnActiveProductId(null);
                          ...
                          setOwnActiveProductId(product.id);
                          setToolType("MEDIA");
                        }
                      }
                    }}
                  ...
                  <Box
                    ...
                    onClick={() => {
                      if (
                        ownEnvProducts[product.id] &&
                        ownEnvProducts[product.id].isEnvironmentProduct
                      ) {
                        setParamsType(
                          ownEnvProducts[product.id].placeHolderId !== undefined
                            ? "PLACEHOLDER"
                            : "CUSTOM"
                        );
                        if (product.id === activeOwnProductId) {
          ...
          >
            <Button
              sx={{
                ...
                backgroundColor:
                  mediaType === "PHOTO" ? "rgb(77, 177, 255)" : "transparent",
                "&:hover": {
                  backgroundColor:
                    mediaType === "PHOTO"
                      ? "rgb(77, 177, 255)"
  ...
  const OwnProductEditor = () => {
    const ownProductEditorRef = useRef<HTMLDivElement>(null);
    const ownProductItemRef = useRef<HTMLDivElement>(null);
    const ownProduct = ownProducts.find(
      (product) => product.id === activeOwnProductId
    );
    const ownEnvProduct = activeOwnProductId
      ? ownEnvProducts[activeOwnProductId]
      : null;

    const ProductPane = () => {
      // Auto compute Product pane height
      const [ownProductPaneHeight, setOwnProductPaneHeight] =
        useState<number>(0);
      useEffect(() => {
        setOwnProductPaneHeight(
          (ownProductEditorRef.current?.clientHeight || 0) -
            (ownProductItemRef.current?.clientHeight || 0)
        );
      }, []);
      if (!activeOwnProductId) return null;
      ...
      const setMediaItem = (type: "MODEL_3D" | "PHOTO", index: number) => {
        ...
        const ownEnvProduct: EnvProduct = {
          id: ownProduct.id,
          type: type,
          imageIndex: type === "PHOTO" ? index : undefined,
          modelIndex: type === "MODEL_3D" ? index : undefined,
        ...
        modifyOwnEnvProduct(ownProduct.id, ownEnvProduct);
      };
      ...
      const MediaContainer = () => {
        return (
          ...
          >
            ...
            >
              {mediaType === "PHOTO" &&
                ownProduct?.images.map((image, index) => {
                  return (
                    <Box
                      sx={{
                        width: "calc(50% - 10px)",
                        ...
                        alignItems: "center",
                        backgroundColor:
                          ownEnvProduct?.imageIndex === index
                            ? "rgba(255, 255, 255, 0.1)"
                            : "rgba(255, 255, 255, 0.03)",
                        border:
                          ownEnvProduct?.imageIndex === index
              ...
              {mediaType === "MODEL_3D" &&
                ownProduct?.models.map((model, index) => {
                  const modelData = {
                    id: model.id,
                    sources: [model.sources && model.sources[0]],
                    alt: "3D Model",
                  ...
                  return (
                    <Box
                      sx={{
                        ...
                        alignItems: "center",
                        backgroundColor:
                          ownEnvProduct?.modelIndex === index
                            ? "rgba(255, 255, 255, 0.1)"
                            : "rgba(255, 255, 255, 0.03)",
                        border:
                          ownEnvProduct?.modelIndex === index
      ...
      return (
        <Box
          sx={{
            ...
            alignItems: "center",
            width: "100%",
            height: `${ownProductPaneHeight}px`,
            backgroundColor: "black",
          }}
          className="ProductPane"
        >
        ...
        >
          <Box
            sx={{
              ...
              justifyContent: "start",
              alignItems: "center",
              backgroundColor: ownEnvProduct?.isEnvironmentProduct
                ? "rgb(10, 10, 10)"
                : "rgb(5, 5, 5)",
              padding: "0 15px 0 15px",
              boxSizing: "border-box",
          ...
          >
            <Checkbox
              ...
              checked={ownEnvProduct?.isEnvironmentProduct || false}
              onChange={(event) => {
                handleCheckboxChange(event, { productId: ownProduct.id });
              }}
              color={"primary"}
            />
            <Box
              component="img"
              src={ownProduct.images[0]?.src}
              sx={{
                height: "60px",
                width: "60px",
                minWidth: "60px",
                backgroundColor: "rgb(255, 255, 255)",
                objectFit: "contain",
                opacity: ownEnvProduct?.isEnvironmentProduct ? 1 : 0.5,
              }}
            />
            <Typography
              sx={{
                ...
                fontFamily: "'Poppins', sans-serif",
                fontWeight: "normal",
                color: ownEnvProduct?.isEnvironmentProduct
                  ? "rgba(255, 255, 255, 0.83)"
                  : "rgba(255, 255, 255, 0.25)",
                textAlign: "left",
                textOverflow: "ellipsis",
            ...
            <Box
              ...
              sx={{
                ...
                height: "20px",
                opacity:
                  ownProduct.id === activeOwnProductId && toolType === "MEDIA"
                    ? 1
                    : ownEnvProduct?.isEnvironmentProduct
                    ? 0.5
                    : 0.2,
                "&:hover": {
                  opacity: ownEnvProduct?.isEnvironmentProduct ? 1 : 0.2,
                  cursor: ownEnvProduct?.isEnvironmentProduct
                    ? "pointer"
                    : "arrow",
                },
              }}
              onClick={() => {
                setMediaType(ownEnvProducts[ownProduct.id]?.type || "PHOTO");
                if (
                  ownProduct.id === activeOwnProductId &&
                  toolType !== "MEDIA"
                ) {
                  setToolType("MEDIA");
                }
            ...
            <Box
              ...
              onClick={() => {
                if (ownEnvProduct?.isEnvironmentProduct) {
                  setParamsType(
                    ownEnvProducts[ownProduct.id].placeHolderId !== undefined
                      ? "PLACEHOLDER"
                      : "CUSTOM"
                  );
                  if (
                    ownProduct.id === activeProductId &&
                    toolType !== "3DPARAMS"
                  ) {
                    if (
  ...
  const FileSelector = () => {
    ...
    const handleDrop = async (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (!brandData) return;
      await AssetService.uploadAssetFiles(
              ...
              >
                <Box
                  sx={{
                    ...
                    justifyContent: "start",
                    alignItems: "center",
                    backgroundColor: envAssets[assetId]?.isEnvironmentAsset
                      ? "rgb(10, 10, 10)"
                      : "rgb(5, 5, 5)",
                    padding: "0 15px 0 15px",
                    boxSizing: "border-box",
                ...
                >
                  <Checkbox
                    ...
                    onChange={(event) => {
                      handleCheckboxChange(event, { assetId: assetId });
                    }}
                    color={"primary"}
                  />
                  <Box
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        cursor: envAsset.isEnvironmentAsset
                          ? "pointer"
                          : "default",
                        textDecoration: envAsset.isEnvironmentAsset
                          ? "underline"
                          : "none",
                      },
                    }}
                    onClick={() => {
                      if (envAsset.isEnvironmentAsset)
                        setActiveAssetId(envAsset.id);
                    }}
                  >
                    {envAsset.name}
  ...
  const AssetEditor = () => {
    ...
    const AssetPane = () => {
      ...
      useEffect(() => {
        setAssetPaneHeight(
          (assetEditorRef.current?.clientHeight || 0) -
            (assetItemRef.current?.clientHeight || 0)
        );
      }, []);
  ...
  return (
    ...
    >
      ...
      {entityType === "OWNASSET" && <OwnProductList />}
      {entityType === "OWNASSET" && <OwnProductEditor />}
      ...
      {entityType === "OWNASSET" && activeOwnProductId && (
        <FullWideButton
          text={"Done"}
          onClick={() => {
            if (!ownEnvProducts[activeOwnProductId]) return null;

            if (
              ownEnvProducts[activeOwnProductId].imageIndex !== undefined ||
              ownEnvProducts[activeOwnProductId].modelIndex !== undefined
            ) {
              setOwnActiveProductId(null);
            } else {
              Swal.fire({
