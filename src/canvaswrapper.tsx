          ...
          await EnvStoreService.getEnvData(brandData.brand_name).then(
            (response) => {
              ...
              for (const envAsset of Object.values(response.envAssets)) {
                // Check if this asset exists in the library with a different ID (without .shackit.com suffix)
                const libraryAssetId = envAsset.id.replace('.shackit.com', '');
                ...
                if (libraryAsset) {
                  ...
                  // But remove the .shackit.com suffix from the ID if present
