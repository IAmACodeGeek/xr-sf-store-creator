...
import { OwnStoreProductService } from "./api/ownStoreShopifyAPIService.js";
import {
  EnvProduct,
  useComponentStore,
  useEnvironmentStore,
...
export default function CanvasWrapper() {
  ...
  const { products, setProducts, ownProducts, setOwnProducts } =
    useComponentStore();
  const { envProducts, setEnvProducts } = useEnvProductStore();
  const { ownEnvProducts, setOwnEnvProducts } = useOwnEnvProductStore();
  const {
    productsLoaded,
    productsLoading,
    setProductsLoaded,
  ...
  const {
    ownstoreproductsLoaded,
    ownstoreproductsLoading,
    setOwnStoreProductsLoaded,
    setOwnStoreProductsLoading,
  } = useResourceFetchStore();
  const {
    envItemsLoaded,
    setEnvItemsLoaded,
  ...
  useEffect(() => {
    ...
    async function fetchOwnStoreProducts() {
      try {
        if (!ownstoreproductsLoaded && !ownstoreproductsLoading && brandData) {
          setOwnStoreProductsLoading(true);
          const response = await OwnStoreProductService.getAllProducts();
          setOwnProducts(response);

          const newOwnEnvProducts: { [id: number]: EnvProduct } = {};
          for (const product of response) {
            newOwnEnvProducts[product.id] = {
              id: product.id,
              type: "PHOTO",
              placeHolderId: undefined,
              imageIndex: undefined,
          ...
          setOwnEnvProducts(newOwnEnvProducts);
          setOwnStoreProductsLoaded(true);
          console.log("All Own Env Products:", response);
        }
      } catch (err) {
        console.error("Own Store Products error:", err);
      }
    }
    ...
    (async () => {
      if (brandData && brandStatus === "VALID") {
        await fetchProducts();
        setProgress(myProgress > 32 ? myProgress : 32);
        await fetchAssets();
        setProgress(myProgress > 58 ? myProgress : 58);
        await fetchEnvData();
        setProgress(myProgress > 76 ? myProgress : 76);
        await fetchModels();
        setProgress(myProgress > 85 ? myProgress : 85);
        await fetchOwnStoreProducts();
